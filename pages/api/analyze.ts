...
export default async function handler(
  ...
  try {
    const { text, url } = req.body as RequestBody;
    if (!text && !url) {
      return res.status(400).json({ error: 'text oder url erforderlich' });
    }
    const result = await analyzeTextServer(text || url || '', url);
    res.status(200).json(result);
  } catch (err) {
    console.error('Fehler bei /api/analyze:', err);
    res.status(500).json({ error: 'Analysefehler: ' + (err as Error).message });
  }
}
